...
export const getCovidInfoFromServer = (dateValue) => async (dispatch) => {
  try {
    ...
    const month = dateObject.month.toString().length === 1
      ? `0${dateObject.month}`
      : dateObject.month;
    const date = dateObject.date.toString().length === 1
      ? `0${dateObject.date}`
      : dateObject.date;
    ...
    const countriesDataArray = Object.values(
      finalResult.dates[dateValue || defaultDate].countries,
    );
