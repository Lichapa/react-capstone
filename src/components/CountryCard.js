...
const CountryCard = ({ className, id }) => {
  ...
  const flagURL = Array.isArray(flags)
    ? flags.filter((country) => {
      ...
      if (
        left.substr(left.indexOf('burm'), 4) === 'burm'
          && right === 'burm'
      ) {
        return true;
      }
    ...
    : [
      {
        flagURL:
            'https://img.etimg.com/thumb/msid-76340801,width-650,imgsize-389237,,resizemode-4,quality-100/coronavirus-vaccine-antibodies1_istock.jpg',
      },
    ];
  ...
  return (
    ...
    >
      <div className={className}>
        <img
          className="flag"
          src={flagURL[0] ? flagURL[0].flagURL : Flag}
          alt="flag"
        />
