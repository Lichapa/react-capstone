...
const HomePage = () => {
  ...
  const worldWideTotal = state.reduce(
    (init, current) => init + current.today_confirmed,
    0,
  );
  ...
  return (
    <div className="home-container">
      ...
      <h1 className="home-heading h-heading">
        {state[0] && new Date(state[0].date).toDateString()}
      </h1>
      ...
      <div className="world-wide">
        ...
        <span>{worldWideTotal.toLocaleString()}</span>
      </div>

      <div className="sticky-inputs">
        <div className="search-container">
          <span className="search-text">Get Data By Date:</span>
          <input
            value={dateValue}
            min="2020-02-01"
        ...
        <div className="search-container search-down">
          <span className="search-text">STATS BY COUNTRY</span>
          <input
            id="country-search"
      ...
      {(internalState.length === 0 ? state : internalState).map(
        (item, index) => {
          if (item === false) {
            return (
              <div key={Math.random()} className="no-match">
                No match found
              </div>
            );
          }
